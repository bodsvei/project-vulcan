...
rospy.init_node('face_speaker_recognition_node', anonymous=True)
speaker_publisher = rospy.Publisher('speakers', String, queue_size=10)
faces_publisher = rospy.Publisher('faces', String, queue_size=10)
...
with mp_face_mesh.FaceMesh(min_detection_confidence=0.5, min_tracking_confidence=0.5, max_num_faces=3) as face_mesh:
    with mp_face_detection.FaceDetection(model_selection=1, min_detection_confidence=0.5) as face_detection:
        while not rospy.is_shutdown():
            ...
            faces_msg = String()
            faces_msg.data = ','.join(map(str, faces))
            faces_publisher.publish(faces_msg)

            
            speakers_msg = String()
            speakers_msg.data = ','.join(map(str, speakers))
            speaker_publisher.publish(speakers_msg)
